...
PreservedAnalyses Loop2SumPass::run(Loop &L, LoopAnalysisManager &LAM,
  ...
  if(BranchInst* Branch = dyn_cast<BranchInst>(Terminator)) {
    if(Branch->isConditional() and Branch->getSuccessor(0)!=Latch
				and Branch->getSuccessor(1)!=Latch){
  ...
  for(Instruction &Inst : *Header) {
    if(PHINode *p = dyn_cast<PHINode>(&Inst)) {
	  if(p == IndVar) foundInd = true;
	  else if(p == AccVar) foundAcc = true;
	  else {
	    return PreservedAnalyses::all();
	  }
	}
  ...
  for(Instruction &I : *Latch) {
    ...
    if(auto *AddInst = dyn_cast<BinaryOperator>(&I)) {
	  if(AddInst->getOpcode() == Instruction::Add) {
		...
		if(Op0 == IndVar and isa<ConstantInt>(Op1)
				and cast<ConstantInt>(Op1)->equalsInt(1)) {
		...
		if(Op1 == IndVar and isa<ConstantInt>(Op0)
				and cast<ConstantInt>(Op0)->equalsInt(1)) {
		  for(int i=0; i<IndVar->getNumIncomingValues(); ++i) {
		    if(IndVar->getIncomingValue(i) == AddInst) {
			  isIncrementByOne = true;
			  check = true;
  ...
  // If the Latch block contains harmful instructions that may cause dependencies, invalid.
  for(Instruction &I : *Latch) {
    if(I.isTerminator()) continue;
	if(I.getOpcode() == Instruction::Store or I.getOpcode() == Instruction::Call or isa<PHINode>(&I)) {
	  return PreservedAnalyses::all();
	}
	// %0 is toxic name
  ...
  // Make a new vectorized block that is connects 7 loops.
  BasicBlock* Vectorized = BasicBlock::Create(Latch->getContext(),
		  Latch->getName() + ".vec", Latch->getParent());

  for(int i=0; i<7; ++i) {
    for(Instruction &Inst : *Latch) {
	  if(BranchInst* Branch = dyn_cast<BranchInst>(&Inst)) break;
  ...
  //Update Induction variable inside Vectorized block
		  ...
		  if(ConstantInt* CI = dyn_cast<ConstantInt>(Op2)) {
			if(CI->equalsInt(1)) {
			  for(Use &U : Inst.operands()) {
			    if(U.get()==OrigIndVar) U.set(CurIndVar);
			  }
  ...
  for(BasicBlock::iterator it = Vectorized->begin();
		  it != Vectorized->end(); ++it) {
    Instruction *CurInst = &(*it);
    string CurName = CurInst->getName().str();
	for(BasicBlock::iterator it2 = next(it);
			it2 != Vectorized->end(); ++it2) {
	  Instruction *NextInst = &(*it2);
	  string NextName = NextInst->getName().str();
	  if(CurName.substr(0, CurName.size()-1)
			  == NextName.substr(0, NextName.size()-1)) break;
	  
	  for(Use &U : NextInst->operands()) {
	    if(U->hasName()) {
		  if(U->getName() == CurName.substr(0, CurName.size()-5)) {
  ...
  if(BitWidth == 1) {
    funcType = FunctionType::get(Type::getInt1Ty(Vectorized->getContext()), argTypes, false);
	M = Vectorized->getModule();
	func = M->getFunction("int_sum_i1");
	if(!func) {
	  func = Function::Create(funcType, Function::ExternalLinkage, "int_sum_i1", M);
	}
  }
  else if(BitWidth == 8) {
    funcType = FunctionType::get(Type::getInt8Ty(Vectorized->getContext()), argTypes, false);
	M = Vectorized->getModule();
	func = M->getFunction("int_sum_i8");
	if(!func) {
	  func = Function::Create(funcType, Function::ExternalLinkage, "int_sum_i8", M);
	}
  }
  else if(BitWidth == 16) {
    funcType = FunctionType::get(Type::getInt16Ty(Vectorized->getContext()), argTypes, false);
	M = Vectorized->getModule();
	func = M->getFunction("int_sum_i16");
	if(!func) {
	  func = Function::Create(funcType, Function::ExternalLinkage, "int_sum_i16", M);
	}
  }
  else if(BitWidth == 32) {
    funcType = FunctionType::get(Type::getInt32Ty(Vectorized->getContext()), argTypes, false);
	M = Vectorized->getModule();
	func = M->getFunction("int_sum_i32");
	if(!func) {
	  func = Function::Create(funcType, Function::ExternalLinkage, "int_sum_i32", M);
	}
 
  }
  else if(BitWidth == 64) {
    funcType = FunctionType::get(Type::getInt64Ty(Vectorized->getContext()), argTypes, false);
	M = Vectorized->getModule();
	func = M->getFunction("int_sum_i64");
	if(!func) {
	  func = Function::Create(funcType, Function::ExternalLinkage, "int_sum_i64", M);
	}
  }
